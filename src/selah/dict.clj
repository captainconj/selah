...
(defn known?
  ...
  ([word vocab]
   (cond
     (= vocab :dict)  (contains? entries word)
     (= vocab :torah) (contains? @torah-vocab word)
     (set? vocab)     (contains? vocab word)
     :else            (contains? entries word))))
...
;; ── Machine-translated Torah vocabulary ─────────────────────────
;;
;; Bulk he→en translations generated by Helsinki-NLP/opus-mt-tc-big-he-en.
;; Static file — no ONNX model needed at runtime. Curated dict entries
;; take priority; this fills the gaps for ~7,000 remaining word forms.

(def ^:private torah-english-path "data/torah-english.edn")

(def ^:private torah-english
  "Lazy cache of machine-translated Hebrew → English for full Torah vocabulary."
  (delay
    (let [f (java.io.File. torah-english-path)]
      (if (.exists f)
        (do (println "[dict] Loading Torah English translations...")
            (clojure.edn/read-string (slurp f)))
        {}))))

(defn translate-english
  "Look up English meaning: curated dict first, then machine-translated cache.
   Returns nil if unknown in both."
  [word]
  (or (get entries word)
      (get @torah-english word)))
